...
model = dict(
    ...
    reg_loss=dict(
        ...
        power=2,
        loss_weight=3e-3),
    cache_size=1520)  # number of training scenes
...
train_cfg = dict(
    ...
    optimizer=dict(type='Adam', lr=3e-3, weight_decay=0.),
...
optimizer = dict(
    diffusion=dict(type='Adam', lr=6e-5, weight_decay=0.),
    decoder=dict(type='Adam', lr=6e-4, weight_decay=0.))
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_prefix='data/abo/tables_train',
        cache_path='data/abo/tables_train_cache.pkl'),
    val_uncond=dict(
        type=dataset_type,
        data_prefix=['data/abo/tables_train', 'data/abo/tables_test'],
        load_imgs=False,
        num_test_imgs=10,
        random_test_imgs=True,
        scene_id_as_name=True,
        max_num_scenes=1000,
        cache_path='data/abo/tables_traintest_cache.pkl'),
...
evaluation = [
    dict(
        ...
        metrics=dict(
            type='FIDKID',
            num_images=1000 * 10,
            inception_pkl='work_dirs/cache/abotables_inception_stylegan.pkl',
            inception_args=dict(
                type='StyleGAN',
